...
import (
	...
	deGet "github.com/deckhouse/deckhouse-cli/internal/dataexport/cmd/get"
...
func init() {
	...
	// TODO rebase on local `DefaultClientConfig` (not virtualization api)
	kubeConfig := kubeclient.DefaultClientConfig(dataCmd.PersistentFlags())
	dataCmd.AddCommand(
		deCreate.NewCommand(kubeConfig),
		deDelete.NewCommand(kubeConfig),
		deGet.NewCommand(kubeConfig),
		deList.NewCommand(kubeConfig),
		deDownload.NewCommand(kubeConfig),
	)

...
var (
	cniSwitchLong = `A group of commands to switch the CNI (Container Network Interface) provider in the Deckhouse cluster.
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, _ []string) {
			...
			if err := cni.RunSwitch(targetCNI); err != nil {
				...
				log.Fatal(wordwrap.WrapString(fmt.Sprintf("❌ Error running switch command: %v", err), 100))
			}
			...
			if err := cni.RunWatch(); err != nil {
				...
				log.Fatal(wordwrap.WrapString(fmt.Sprintf("❌ Error monitoring switch progress: %v", err), 100))
			}
		},
	}
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, _ []string) {
			if err := cni.RunWatch(); err != nil {
				...
				log.Fatal(wordwrap.WrapString(fmt.Sprintf("❌ Error running watch command: %v", err), 100))
			}
		},
	}
	return cmd
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, _ []string) {
			if err := cni.RunCleanup(); err != nil {
				log.Fatal(wordwrap.WrapString(fmt.Sprintf("❌ Error running cleanup command: %v", err), 100))
			}
		},
	}
	return cmd

...
- **Шаг 0 (§3).** Модель `Node` (structured `SourceRef`, singular `Data`, **UID самого
  snapshot-CR**), идентичности: `CanonicalSnapshotIdentity`
  (непрозрачный внутренний ключ resume/checksum + collision-дискриминатор — **не путь**) и
  `CanonicalSourceIdentity` (provenance). Имя каталога — существующая `archive.NodeDirName`
  (Вариант A, §10; отдельная `ArchiveNodeDirName` удалена). Различие `status.sourceRef` vs
  `status.data.source`, политика «старое не
...
2. **`status.data` — singular object (Variant A, ≤1 на узел)**, пишет ядро (зеркалит из
   `SnapshotContent.status.data`); source-based форма
   `{source, artifact, volumeMode, fsType, accessModes, storageClassName, size}`. 2.md,
   1909–1928. Существующий CLI-тип `internal/snapshot/api/v1alpha1.SnapshotDataBinding`
   (`target`/`targetUID`/`dataRef`) **устарел** и этой форме не соответствует; Stage 2
   использует `source.NodeData` (`source/status.go`), а legacy-тип удаляется после перевода
   downstream (§10).
...
4. **Идентичность источника — `status.sourceRef`** — `{apiVersion,kind,name,uid[,namespace]}`,
   пишет домен. 2.md, 1900–1908. Совпадает с `SourceRefIdentity` (`source/source_ref.go:27-33`).
   `uid` обязателен всегда; `namespace` — для namespaced source kinds, но отсутствует у
   cluster-scoped корня `v1/Namespace` (§3.5).
...
type Node struct {
    ...
    // nil у агрегатора/manifest-only узла. Source-based тип source.NodeData (НЕ legacy
    // internal/snapshot/api/v1alpha1.SnapshotDataBinding, который устарел, §10).
    Data *NodeData
...
```go
// dataListFromStatusData — compatibility adapter под старый downstream, ожидающий срез.
// НЕ новая модель: Variant A закрепляет singular Data (максимум один биндинг).
func dataListFromStatusData(data *NodeData) []*NodeData {
    if data == nil {
        return nil
    }
...
> **ОБНОВЛЕНО (решение по ревью, см. §10): отдельная `ArchiveNodeDirName` удалена — Вариант A.**
> Имя каталога НЕ вводит новую сущность: layout остаётся источником истины `archive.NodeDirName`
> (`<kindlower>_<name>`) + `CollisionNodeDir` (`__<short>`). Так исключаем случайное подключение
> параллельного API и смену формата архива.

`CanonicalSnapshotIdentity`/`CanonicalSourceIdentity` — детерминированные (фиксированный
порядок, разделитель `\x00`), с unit-тестами на стабильность. Применение:

- **resume-ключ / checksum / индекс узла** → `CanonicalSnapshotIdentity` (внутренний ключ,
  **не** попадает в filesystem path);
- **имя каталога узла в архиве** → существующая `archive.NodeDirName`: читаемая база =
  `SourceRef.Name` (если `SourceRef != nil`, иначе `Node.Name`); collision-суффикс `__<short>`
  из короткого hash `CanonicalSnapshotIdentity`. raw canonical key в путь не кладём;
- **provenance захваченного объекта** → `CanonicalSourceIdentity` (по `SourceRef`);
- **data provenance** → по `Data.Source` (при необходимости).

> Новый layout не изобретаем (Вариант A): формат имён каталогов сохраняется существующий, с
> существующим collision-механизмом (`__<short>`), где `<short>` берётся из
> `CanonicalSnapshotIdentity`. Дерево каталогов не превращается в UID/hash — читаемая часть
> имени (source name) остаётся понятной человеку.
...
- **Требуется стабильность нового формата между запусками новой версии**:
  `CanonicalSnapshotIdentity`/`CanonicalSourceIdentity` детерминированы (unit-тест). Риск —
  не «сломаем старое», а «сделаем новый resume недетерминированным».

### 3.5 Парсинг namespaced status — typed через converter, fail-closed

Узлы тянутся `fetchUnstructured` (discovery динамический ради доменных kind). Source-based
typed-модель (`source.SourceRefIdentity` и `source.NodeData`) декодируется единой
typed-функцией через `runtime.DefaultUnstructuredConverter.FromUnstructured` для фрагментов
status:
...
func ParseNodeStatus(obj *unstructured.Unstructured) (
    ident SnapshotIdentity, // из metadata (apiVersion/kind/namespace/name/uid), валидируется
    src *SourceRefIdentity,  // status.sourceRef или nil
    data *NodeData,          // status.data или nil (source-based, НЕ legacy SnapshotDataBinding)
    err error,
)
```
...
- **`SnapshotIdentity` узла:** `apiVersion`/`kind`/`namespace`/`name`/`uid` обязательны и
  проверяются в начале (identity питает resume/checksum/collision — слабая identity недопустима).
  Snapshot-узлы на Этапе 2 namespaced, поэтому `metadata.namespace` обязателен.
...
- **namespace в `status.sourceRef` — зависит от scope источника:** для namespaced source kinds
  `namespace` обязателен; **исключение — cluster-scoped корневой источник `v1/Namespace`**
  (root capture-Snapshot), у которого `sourceRef` намеренно без `namespace` (2.md). Проверка
  per-scope (`sourceRefRequiresNamespace`), без RESTMapper. Прочие cluster-scoped source kinds
  **вне области Этапа 2**.
...
4. Downstream (`pipeline`, `describe`, `volume/manifest_worker.go`, `archive/resume.go`)
   ...
   `CanonicalSnapshotIdentity`; имя каталога — существующая `archive.NodeDirName`** (Вариант A,
   §3.2/§10): читаемая база = `SourceRef.Name`/`Node.Name`, collision `__<short>` из
   `CanonicalSnapshotIdentity`. Raw canonical key в путь не кладём; layout не меняем.
...
11. `CanonicalSnapshotIdentity`/`CanonicalSourceIdentity` детерминированы между запусками
    (стабильность нового формата; совместимость со старым не проверяется — §3.4).
12. Layout архива (Вариант A): имя каталога — существующая `archive.NodeDirName`
    (`<kindlower>_<name>`), читаемая база = `SourceRef.Name`/`Node.Name`; при коллизии базовых
    имён каталоги различимы суффиксом `__<short>` из `CanonicalSnapshotIdentity`. Raw canonical
    key в путь не попадает; формат имён каталогов на диске не меняется.
...
1. Шаг 0 (§3): модель `Node` + `UID`; идентичности `CanonicalSnapshotIdentity` (ключ +
   collision-дискриминатор) / `CanonicalSourceIdentity` (provenance); имя каталога —
   существующая `archive.NodeDirName` (Вариант A, §10); политика совместимости; различие
...
5. Downstream на новую модель `Node` (напрямую/adapter); resume-ключ — по
   `CanonicalSnapshotIdentity`; имя каталога — существующая схема `archive.NodeDirName`
   (читаемая база = `SourceRef.Name`/`Node.Name`, collision `__<short>` из
   `CanonicalSnapshotIdentity`), layout не меняем (Вариант A, см. §10).
...
- **`status.sourceRef` = `SnapshotSourceObjectRef`** `{apiVersion,kind,name,uid[,namespace]}`
  — совпадает с CLI `SourceRefIdentity`. `uid` **нормативен и обязателен** в `ParseNodeStatus`
  (в отличие от облегчённого `spec.sourceRef` `{apiVersion,kind,name}`). **`namespace` —
  scope-зависим:** обязателен для namespaced source kinds, но **намеренно отсутствует** у
  cluster-scoped корневого источника `v1/Namespace` (2.md, root capture-Snapshot). Поэтому
  проверка namespace — per-scope (`sourceRefRequiresNamespace`), а не безусловная (ранний
  вывод «все примеры содержат namespace» был неверен). Полнота data-ноги
  (`status.data.source.uid`, artifact) проверяется отдельно в `RequireNodeData` на Ready-узле.
- **`SnapshotIdentity` самого узла валидируется в `ParseNodeStatus`** (fail-closed:
  `apiVersion`/`kind`/`namespace`/`name`/`uid`) — она питает resume/checksum/collision, поэтому
  слабая (частично пустая) identity недопустима даже при отсутствующих `sourceRef`/`data`.
- **Layout каталогов архива НЕ меняется (решение: Вариант A).** 2b снимает зависимость от
  `SnapshotContent`, но НЕ меняет формат архива на диске. Источник истины остаётся
  `archive.NodeDirName(kind,name)` = `<kindlower>_<name>` (`names.go:166`) + `CollisionNodeDir`
  (`__<short>`, `writer.go:37`); корень — пользовательский путь (`ScanAbsolute`).
  Преждевременный `source.ArchiveNodeDirName` (`<kindlower>-<name>-<hash8>`) **удалён**, чтобы
  его случайно не подключили и не поменяли layout. `SnapshotIdentity` /
  `CanonicalSnapshotIdentity` / `CanonicalSourceIdentity` оставлены — это resume-identity и
  вход для collision-дискриминатора.
- **Правила именования каталогов в 2b (при переходе с legacy `SourceName` на structured
  `SourceRef`):**
  - читаемая база каталога = `SourceRef.Name`, если `SourceRef != nil`, иначе `Node.Name`
    (root и узлы без source используют CR name);
  - идентичность узла и resume — по `SnapshotIdentity` (включая `UID`), а не по source name;
  - collision-суффикс `__<short>` строить из короткого хэша `CanonicalSnapshotIdentity`
    (два узла с одинаковым source name не смешиваются);
  - `archive.NodeDirName` / `CollisionNodeDir` остаются функциями layout как есть; меняется
    только откуда берётся читаемая база (`SourceRef.Name`) и из чего вычисляется `<short>`.

...
func TestImageBuilder_Build(t *testing.T) {
	...
	files, err := stub.ExtractFiles(img)
	require.NoError(t, err)
...
func TestImageBuilder_WithPlatform_Defaults(t *testing.T) {
	img, err := stub.NewImageBuilder().Build()
	require.NoError(t, err)
...
func TestImageBuilder_MustBuild_Success(t *testing.T) {
	assert.NotPanics(t, func() {
		img := stub.NewImageBuilder().WithFile("a.txt", "b").MustBuild()
		require.NotNil(t, img)
	})
}
...
func TestImageBuilder_Digest(t *testing.T) {
	img1 := stub.NewImageBuilder().WithFile("f.txt", "same").MustBuild()
	img2 := stub.NewImageBuilder().WithFile("f.txt", "same").MustBuild()
	img3 := stub.NewImageBuilder().WithFile("f.txt", "different").MustBuild()
...
func TestMustMarshalJSON(t *testing.T) {
	...
	result := stub.MustMarshalJSON(payload{Version: "v1.0.0", Count: 42})

...
func NewClient(flags ...*pflag.FlagSet) (*Client, error) {
	...
	return &Client{restConfig: restConfig}, nil
}

// NewClientForConfig derives a data-plane client from the command-scoped REST
// configuration without parsing kubeconfig a second time.
...
func (c *Client) newPersistentHTTPClientForOrigin(origin httpOrigin) (*PersistentHTTPClient, error) {
...
func (c *PersistentHTTPClient) HTTPDo(req *http.Request) (*http.Response, error) {
	...
	if req.Header.Get("Authorization") == "" && !c.hasConfiguredAuth && c.requireAuth {
...
// explicitly; Client's own default is unchanged for every other caller of
// NewClient that never calls it.
...
// requests. Streaming callers can clear it on a copied Client before building
...
// NewRuntimeClient builds a controller-runtime client from the same tuned REST
// configuration used by the command's other Kubernetes clients.
func NewRuntimeClient(
	...
	kubeRtClient, err := ctrlrtclient.New(config, clientOpts)
	if err != nil {
...
// NewRTClient builds a controller-runtime client from this client's REST
// configuration. New command code should prefer NewRuntimeClient with its
...
// than replacing it, and a Client that never calls it keeps its previous
...
// or depending on the Client's auth handling.

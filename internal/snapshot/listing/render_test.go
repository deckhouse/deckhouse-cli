...
func TestRenderHuman_DedupChildObjects(t *testing.T) {
	tree := &listing.Tree{
		Source: listing.Source{
			Kind:      "cluster",
			Cluster:   "https://example.com:6443",
		...
		Root: &listing.NodeView{
			...
			Objects: []listing.ObjectView{
				{APIVersion: "v1", Kind: "ConfigMap", Name: "cm1", Namespace: "demo"},
				{APIVersion: "v1", Kind: "Secret", Name: "sec1", Namespace: "demo"},
			},
			Children: []*listing.NodeView{
				{
					ID:          "Snapshot--child",
	...
	if err := listing.Render(&buf, tree, listing.FormatHuman); err != nil {
		t.Fatalf("Render human (dedup): %v", err)
	}

	out := buf.String()

	// ConfigMap is exclusive to root — must appear.
	if !strings.Contains(out, "ConfigMap") {
		t.Fatalf("expected ConfigMap in output:\n%s", out)
	}

	// Secret is covered by the child — must appear exactly once (under child).
	if count := strings.Count(out, "Secret"); count != 1 {
		t.Fatalf("Secret should appear exactly once (deduped from root), got %d:\n%s", count, out)
	}
	...
	if childIdx < 0 {
		t.Fatalf("child node line not found:\n%s", out)
	}

	// The line immediately after the child header must show the Secret.
	if childIdx+1 >= len(lines) || !strings.Contains(lines[childIdx+1], "Secret") {
		t.Fatalf("expected Secret on line after child node, got:\n%s", out)
	}
}

...
**d8 CLI binary:** picking the newest published version needs the tag listing, which a proxy registry does not serve, so the binary is skipped in this mode unless the version is pinned: `--deckhouse-cli-tag vX.Y.Z`.

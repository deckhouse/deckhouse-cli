...
func TestModuleFilter_Match(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			f:    map[string][]string{},
			args: args{mod: modules.Module{Name: "test", RegistryPath: "registry.example.com/dh", Releases: []string{"v1.2.3"}}},
			want: true,
		},
		{
		...
		},
		{
			...
			f:    map[string][]string{"deckhouse-admin": {"v1.2.0", "v1.2.1", "v1.2.2", "v1.2.3"}},
			args: args{mod: modules.Module{Name: "deckhouse-admin", RegistryPath: "registry.example.com/dh", Releases: []string{"v1.2.3"}}},
			want: true,
		},
		{
			name: "Happy path, module unknown to filter",
			f:    map[string][]string{"deckhouse-admin": {"v1.2.0", "v1.2.1", "v1.2.2", "v1.2.3"}},
			args: args{mod: modules.Module{Name: "op-monitoring", RegistryPath: "registry.example.com/dh", Releases: []string{"v1.2.3"}}},
			want: false,
		},
	}

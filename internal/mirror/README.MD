...
Every mirrored channel is also written into the bundle as a tag alias of the version it resolves to, in `release-channel`, in the repo root and in `install` — the same repositories that carry channel tags upstream (`install-standalone` carries none). So after `d8 mirror push` the destination registry serves `<repo>:stable` and `<repo>/install:stable` exactly like the source does, and the air-gapped install flow `docker run <repo>/install:<channel>` works unchanged. Aliases are extra index entries on images already in the bundle: no extra download, and no requirement that the source registry publish `<repo>/install:<channel>` itself.

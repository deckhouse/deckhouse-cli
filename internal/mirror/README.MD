...
d8 CLI plugins that the mirrored modules need are selected **automatically** (see [Plugin Mirroring](#plugin-mirroring)); the flag adds more on top. The d8 binary itself is mirrored at its newest published version unless a version is named (see [Deckhouse CLI Mirroring](#deckhouse-cli-mirroring)).
...
| `--deckhouse-cli-tag` | Specific d8 CLI version to mirror, e.g. `v0.13.1` (default: the newest published stable version) |
...
Every mirrored channel is also written into the bundle as a tag alias of the version it resolves to, in `release-channel` and in `install`. So after `d8 mirror push` the air-gapped install flow `docker run <repo>/install:<channel>` works against the mirror unchanged. Aliases are extra index entries on images already in the bundle: no extra download, and no requirement that the source registry publish `<repo>/install:<channel>` itself. The main Deckhouse repository and `install-standalone` keep version tags only — they are addressed by version, and channel metadata lives in `release-channel`.
...
- **All release channels in the bundle are re-pointed to the pinned build.** If the source registry serves a release-channel image for the value, every default channel (`alpha`, `beta`, `early-access`, `stable`, `rock-solid`) in the resulting bundle is aliased to it, in `release-channel` and in `install`. After push, a cluster on any channel receives the pinned version, and `docker run <repo>/install:<channel>` runs the pinned installer. This aliasing only re-tags an already-downloaded image — it pulls no extra data. For a channel-name value this is what makes "`stable` only" work end to end; for a custom tag it happens only when such a release-channel image exists.
...
### Deckhouse CLI Mirroring

The d8 binary is published as a multi-platform OCI index at the bare registry root, in the repository that owns the plugins catalog:

| Path | Contents |
|------|----------|
| `<root>/deckhouse-cli:<vX.Y.Z>` | One CLI release (linux/darwin/windows in one index) |
| `<root>/deckhouse-cli/plugins` | The plugin catalog under it (see [Plugin Mirroring](#plugin-mirroring)) |

**Bundle output:** `deckhouse-cli.tar`, carrying the layout at `deckhouse-cli/`.

One version is mirrored: the newest published **stable** one (non-semver build tags and genuine pre-releases are dropped, the same notion of stable that `d8 dist update` uses), or the exact version named by `--deckhouse-cli-tag`. After `d8 mirror push` it lands at `<target>/deckhouse-cli` - where the in-cluster registry-packages-proxy looks - so `d8 dist update` works in the air-gapped cluster with no extra setup.

**A missing binary never fails the pull.** Registries that publish no `deckhouse-cli` repository, or deny access to it, are reported in the summary (`d8 dist: not mirrored - not available in this registry`) and the rest of the bundle is built as usual. The exception is `--deckhouse-cli-tag`: a version asked for by name and not found fails the pull, like an unmet `--include-plugin`.

Release metadata (`<root>/deckhouse-cli/version:<vX.Y.Z>`) is **not** mirrored - the registries do not publish it yet.
...
├── deckhouse-cli.tar         # The d8 binary itself (see Deckhouse CLI Mirroring)
...
After a pull, `d8 mirror` prints a framed summary of what was pulled (platform, installer, security databases, modules, packages, the d8 CLI and its plugins) and the bundle size.

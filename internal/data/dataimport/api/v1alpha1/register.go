...
// AddKnownTypes registers the DataImport types under SchemeGroupVersion (storage-foundation).
...
//
// One Go type serves both producers: the spec carries both destination shapes and fills the one
// the resolved backend understands (see DataImportSpec). Registering one scheme per resolved
// group keeps exactly one GroupVersionKind mapped to each type, which is what the
// controller-runtime client requires to address the object at all.

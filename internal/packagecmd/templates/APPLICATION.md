...
- Аргумент — `list`: первый элемент контекст (`.`), второй — имя компонента (опционально).
- Берёт `.Platform.applications.publicDomainTemplate` и подставляет два `%s` **строго в порядке**:
  `.Application.Instance.Name`, `.Application.Instance.Namespace`.
- Если компонент задан, то подставляет его в начало получившегося домена: Компонент.Домен.
- Если в шаблоне платформы не ровно два `%s` — прерывает рендер понятной ошибкой (`fail`).

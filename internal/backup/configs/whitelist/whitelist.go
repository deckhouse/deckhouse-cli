...
func (f *BakedInFilter) Matches(obj runtime.Object) bool {
	...
	f.initOnce.Do(func() {
		if err := yaml.Unmarshal(rawBuiltInWhitelist, &f.list); err != nil {
			panic(err)
		}
	})

	if !lo.HasKey(f.list, namespace) ||
		!lo.HasKey(f.list[namespace], apiVersion) ||
		!lo.HasKey(f.list[namespace][apiVersion], kind) {
		return false
	}

	_, foundInWhitelist := lo.Find(f.list[namespace][apiVersion][kind], func(item string) bool {

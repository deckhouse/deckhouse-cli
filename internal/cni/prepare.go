...
func generateWebhookCertificates(namespace string) (caCert, serverCert, serverKey []byte, err error) {
	caSerialNumberLimit := new(big.Int).Lsh(big.NewInt(1), 128) // 128-bit random for CA serial
	...
	serverSerialNumberLimit := new(big.Int).Lsh(big.NewInt(1), 128) // 128-bit random for Server serial
